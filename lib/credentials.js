...
function createCredentialStore({ safeStorage, authPath, platform = process.platform, io = fs }) {
   ...
   * @param {string} record.password      Still kept: the in-game AuthMe
   *   handshake needs it. Staying logged in no longer does.
   * @param {string|null} [record.token]        Short-lived API session token.
   * @param {string|null} [record.refreshToken] Exchanged for a new session
   *   without re-sending the password.
   */
  function save({ username, password, token = null, refreshToken = null }) {
    try {
      io.mkdirSync(path.dirname(authPath), { recursive: true });
    } catch {}
    io.writeFileSync(
      authPath,
      safeStorage.encryptString(JSON.stringify({ username, password, token, refreshToken })),
    );
  }
