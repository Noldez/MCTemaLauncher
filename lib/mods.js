 ...
 */
/**
 * True for a bare file name - no directory part, no traversal, no drive or
 * UNC prefix. Guards every path built from stored (renderer-writable) state.
 * @param {unknown} name
 */
...
function stageMods({ srcDir, dstDir, hashes, optionalDir, optionalMods = [], io = fs }) {
  ...
  for (const st of optionalMods || []) {
    ...
    // The filename comes from the settings file, which the renderer can write
    // through config:set. Anything but a plain name would let a join() escape
    // the mods directories entirely, so it is refused rather than sanitized.
    if (!isPlainFileName(st.file)) continue;
...
module.exports = { sha256File, stageMods, resolveJava, isPlainFileName };
