...
class MCLCore extends EventEmitter {
  async launch (options) {
    try {
      ...
      // MCTema: was `if (!fs.existsSync(mcPath))`, which trusted any file that
      // happened to be there. A half-written jar then broke every launch
      // forever. ensureJar checks it against the published hash instead.
      await this.handler.ensureJar(mcPath)
  ...
  printVersion () {
    this.emit('debug', '[MCLC]: vendored 3.18.2 (fetch port)')
  }
    ...
    const minecraft = child.spawn(this.options.javaPath ? this.options.javaPath : 'java', launchArguments,
      {
        ...
        // MCTema: extra env for the game only. Putting a secret in the
        // launcher's own process.env would expose it to every child spawned
        // during preparation, and to any same-user process meanwhile.
        env: { ...process.env, ...(this.options.overrides.env || {}) }
      })
