// Vendored from minecraft-launcher-core 3.18.2 (MIT, (c) Pierce Harriz).
// Network layer ported from the deprecated `request` package to node http(s);
// `checksum` package replaced with node crypto. No other behavioral changes.
const fs = require('fs')
...
class Handler {
  constructor (client) {
    ...
    this.timeout = this.options.timeout || 50000
    const maxSockets = this.options.overrides.maxSockets || 2
  ...
  _getStream (url, redirects = 0) {
    return new Promise((resolve, reject) => {
      const mod = url.startsWith('http:') ? http : https
      const agent = url.startsWith('http:') ? this.httpAgent : this.httpsAgent
      const req = mod.get(url, { agent }, (res) => {
        if (res.statusCode >= 300 && res.statusCode < 400 && res.headers.location && redirects < MAX_REDIRECTS) {
          ...
          return this._getStream(next, redirects + 1).then(resolve, reject)
        }
        resolve(res)
      })
      req.setTimeout(this.timeout, () => req.destroy(new Error(`Request timeout for ${url}`)))
      req.on('error', reject)
    })
  }

  // GET a URL and buffer the body as a string. Rejects with the socket error
  // (its .code is preserved so ENOTFOUND cache fallbacks keep working).
  async _getText (url) {
    const res = await this._getStream(url)
    return new Promise((resolve, reject) => {
      let body = ''
  ...
  checkSum (hash, file) {
    return new Promise((resolve) => {
      ...
      stream.on('data', (d) => sha1.update(d))
      stream.on('end', () => resolve(hash === sha1.digest('hex')))
      stream.on('error', (err) => {
  ...
  getVersion () {
    return new Promise((resolve) => {
      (async () => {
        ...
        let body = null
        try {
          body = (await this._getText(manifest)).body
        } catch (e) {
          error = e
        }
        ...
        if (desiredVersion) {
          ...
          try {
            versionBody = (await this._getText(desiredVersion.url)).body
          } catch (e) {
            versionError = e
          }
          if (versionError && versionError.code !== 'ENOTFOUND') throw Error(versionError)
          if (!versionError) {
            fs.writeFile(path.join(`${cache}/${this.options.version.number}.json`), versionBody, (err) => {
  ...
  /**
   * MCTema: make sure the jar on disk is the one Mojang published.
   *
   * Upstream only downloads when the file is missing, so a download cut short
   * by a dropped connection, a full disk or antivirus leaves a partial jar that
   * is never looked at again - every later launch dies inside Fabric with
   * "zip END header not found" and no amount of reinstalling the launcher helps,
   * because the broken file lives in the game directory. Libraries already
   * verify what is on disk; this brings the version jar in line.
   */
  async ensureJar (mcPath) {
    const sha1 = this.version.downloads && this.version.downloads.client
      ? this.version.downloads.client.sha1
      : null
    if (fs.existsSync(mcPath)) {
      if (!sha1 || await this.checkSum(sha1, mcPath)) return
      this.client.emit('debug', '[MCLC]: Version jar is damaged, downloading it again')
      try { fs.rmSync(mcPath, { force: true }) } catch {}
    }
    await this.getJar()
  }

  async getJar () {
    const jarName = `${this.options.version.custom ? this.options.version.custom : this.options.version.number}.jar`
    await this.downloadAsync(this.version.downloads.client.url, this.options.directory, jarName, true, 'version-jar')
    // MCTema: verify the client jar against the manifest hash before it is run.
    const clientSha1 = this.version.downloads.client.sha1
    if (clientSha1 && !await this.checkSum(clientSha1, path.join(this.options.directory, jarName))) {
      throw new Error('Client jar failed checksum verification')
    }
  ...
  async getForgedWrapped () {
    ...
    if (this.isModernForge(json)) {
      ...
      await Promise.all(json.libraries.map(async library => {
        ...
        try {
          const res = await this._getStream(downloadLink)
          res.resume()
          if (res.statusCode === 404) library.url = this.options.overrides.url.fallbackMaven
        } catch (e) {
  ...
  async downloadToDirectory (directory, libraries, eventName) {
    ...
    await Promise.all(libraries.map(async library => {
      ...
      // MCTema: jarPath and name come from remote JSON. Refuse any value that
      // resolves outside the libraries directory, or a filename with a
      // separator in it - otherwise a hostile manifest writes anywhere.
      const root = path.resolve(directory)
      if (!path.resolve(jarPath).startsWith(root + path.sep) && path.resolve(jarPath) !== root) {
        throw new Error(`Library path escapes the libraries directory: ${library.name}`)
      }
      if (/[/\\]|^\.\.?$/.test(name)) {
        throw new Error(`Unsafe library filename: ${name}`)
      }
      ...
      if (library.downloads && library.downloads.artifact && library.downloads.artifact.sha1) {
        ...
        if (!await this.checkSum(library.downloads.artifact.sha1, target)) {
          await downloadLibrary(library)
          // MCTema: upstream only re-downloaded and carried on. A library that
          // still fails its published hash is refused - it ends up on the game
          // classpath, so a mismatch means running code Mojang did not publish.
          if (!await this.checkSum(library.downloads.artifact.sha1, target)) {
            throw new Error(`Library failed checksum verification: ${name}`)
          }
        }
      }
