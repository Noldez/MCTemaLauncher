...
/**
 * Strip the player's identity out of a log before it is uploaded. The launch
 * line contains absolute paths, so the Windows/Linux account name would ride
 * along to our server with every report; staff only ever need the stacktrace.
 * @param {unknown} logText
 * @param {string} [homeDir]
 * @param {string} [userName]
 */
function redactLog(logText, homeDir, userName) {
  let out = String(logText || '');
  if (homeDir) {
  ...
  if (userName && userName.length > 2) {
    ...
    out = out.replace(new RegExp(escaped, 'gi'), 'USER');
  }
  return out;
}

module.exports = { createLogBuffer, suspectCause, redactLog };
